...
/**
 * Navbar reverted to the prior compact arrangement:
 * - Single gradient bar with centered primary links on md+ and a compact overflow on mobile.
 * - Keeps existing colors/gradients; only adjusts border radius to rounded-xl consistently.
 * - No functional or routing behavior changes.
 */
export default function Navbar({ theme, onToggle }) {
  const baseLink =
    "px-3 py-2 rounded-xl text-sm font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50";
  const activeLink =
  ...
  return (
    <nav className="sticky top-0 z-40" aria-label="Site">
      <div className="w-full app-header-major rounded-xl">
